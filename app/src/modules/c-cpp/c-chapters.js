...
},
{
  id: 18,
  title: "现代C编程实践",
  description: "安全编码、现代构建工具和C11/C17新特性",
  content: `<h2>现代C编程实践</h2>

    <h3>一、安全编码</h3>
    <p>C语言赋予程序员直接操控内存的能力，但也带来了<strong>缓冲区溢出</strong>、<strong>整数溢出</strong>等安全隐患。现代C开发强调使用安全函数（如 <code>snprintf</code> 替代 <code>sprintf</code>）、静态分析工具和运行时检测工具（如 AddressSanitizer）来防范这些问题。</p>

    <h3>二、CMake 构建系统</h3>
    <p><strong>CMake</strong> 是跨平台的构建系统生成器，通过 <code>CMakeLists.txt</code> 描述项目结构，然后生成对应平台的构建文件（如 Makefile 或 Visual Studio 工程）。推荐使用 <strong>out-of-source build</strong>（在独立的 build 目录中构建），保持源码目录整洁。</p>
...
target_link_libraries(myapp m)
    ...
    <h3>三、C11/C17 新特性</h3>
    <p>C11 引入了许多重要特性：<code>_Static_assert</code> 编译期断言、<code>_Generic</code> 类型泛型表达式、<code>_Atomic</code> 原子操作、匿名结构体/联合体、<code>_Alignof</code>/<code>_Alignas</code> 对齐控制、<code>_Noreturn</code> 函数修饰符等。C17 主要修复了 C11 的缺陷，未引入新特性。</p>

    <div class="key-point">
      <strong>核心原则：</strong>现代C编程追求在保持C语言高性能优势的同时，通过工具链和语言新特性减少未定义行为和安全漏洞，写出更健壮、更可维护的代码。
    </div>
  `,
  questions: [
    {
      type: "choice",
      question: "以下哪个函数可以有效防止缓冲区溢出？",
      options: [
        "sprintf",
        "snprintf",
        "strcat",
      ...
      answer: 1,
      explanation: "<code>snprintf</code> 接受一个 <code>size</code> 参数来限制写入的最大字符数，从而防止缓冲区溢出。<code>sprintf</code>、<code>strcat</code>、<code>gets</code> 都不检查目标缓冲区大小，极易导致溢出。"
    },
    {
      type: "truefalse",
      question: "缓冲区溢出漏洞可以被攻击者利用来执行任意代码。",
      options: ["正确", "错误"],
      answer: 0,
      explanation: "缓冲区溢出可以覆盖栈上的返回地址或函数指针，攻击者精心构造溢出数据即可劫持程序控制流，执行任意代码。这是最经典的安全漏洞之一。"
    },
    {
      type: "choice",
      question: "以下代码存在什么问题？<pre class=\"language-c\">unsigned char x = 255;\nx = x + 1;\nprintf(\"%d\", x);</pre>",
      options: [
        "语法错误",
        "无符号整数回绕，x 变为 0",
        "程序崩溃",
      ...
      answer: 1,
      explanation: "<code>unsigned char</code> 范围是 0~255。255 + 1 超出范围后发生<strong>无符号回绕</strong>（wraparound），结果为 0。无符号整数溢出在C标准中是定义行为（回绕），但有符号整数溢出是未定义行为。"
    },
    {
      type: "choice",
      question: "静态分析工具的主要作用是：",
      options: [
        "在运行时检测内存越界",
        "在不运行程序的情况下分析源代码中的潜在缺陷",
        "自动修复所有 bug",
        "替代编译器进行代码编译"
      ],
      answer: 1,
      explanation: "静态分析工具（如 Clang Static Analyzer、Cppcheck）在<strong>编译前</strong>扫描源代码，找出潜在的空指针解引用、内存泄漏、未初始化变量等问题，无需运行程序即可发现缺陷。"
    },
    {
      type: "choice",
      question: "AddressSanitizer（ASan）属于什么类型的检测工具？",
      options: [
        "静态分析工具，编译前检查源代码",
        "运行时检测工具，在程序执行时发现内存错误",
        "代码格式化工具",
        "性能分析工具"
      ],
      answer: 1,
      explanation: "ASan 是<strong>运行时</strong>内存错误检测工具，通过在编译时插桩（<code>-fsanitize=address</code>），在程序运行时检测堆栈缓冲区溢出、use-after-free、内存泄漏等问题。UBSan 则用于检测未定义行为。"
    },
    {
      type: "choice",
      question: "关于 CMake，以下描述最准确的是：",
      options: [
        "CMake 是直接编译C代码的编译器",
        "CMake 是跨平台的构建系统生成器，生成 Makefile 等构建文件",
        "CMake 只能在 Linux 上使用",
        "CMake 是 Make 的新版本"
      ],
      answer: 1,
      explanation: "CMake 本身不编译代码，而是根据 <code>CMakeLists.txt</code> 生成对应平台的构建文件（Linux 上生成 Makefile，Windows 上可生成 Visual Studio 工程等），然后由实际的构建工具执行编译。"
    },
    {
      type: "choice",
      question: "一个最小的 CMakeLists.txt 通常包含以下哪三个基本命令？",
      options: [
        "cmake_minimum_required、project、add_executable",
        "include、define、compile",
        "gcc、-o、-Wall",
        "make、build、install"
      ],
      answer: 0,
      explanation: "最基本的 CMakeLists.txt 需要：<code>cmake_minimum_required</code> 指定最低 CMake 版本、<code>project</code> 定义项目名称、<code>add_executable</code> 指定生成的可执行文件及其源文件。"
    },
    {
      type: "truefalse",
      question: "CMake 中 <code>target_link_libraries</code> 用于为目标链接第三方库或系统库。",
      options: ["正确", "错误"],
      answer: 0,
      explanation: "<code>target_link_libraries(myapp m)</code> 表示将数学库 <code>libm</code> 链接到目标 <code>myapp</code>。此命令也可链接其他 CMake 目标或第三方库，并自动传递包含路径等属性。"
    },
    {
      type: "choice",
      question: "CMake 推荐的 out-of-source build 是什么意思？",
      options: [
        "不使用源代码进行构建",
        "在源码目录之外的独立目录中执行构建",
        "从远程服务器下载源码后构建",
      ...
      answer: 1,
      explanation: "out-of-source build 指在源码目录外创建单独的 <code>build</code> 目录（如 <code>mkdir build && cd build && cmake ..</code>），所有构建产物都在该目录中，保持源码目录干净，便于清理和管理多种构建配置。"
    },
    {
      type: "choice",
      question: "C11 中 <code>_Static_assert</code> 的作用是：",
      options: [
        "在运行时检查条件是否成立",
        "在编译时检查常量表达式条件，不满足则编译失败",
        "在预处理阶段展开宏",
        "定义静态变量"
      ],
      answer: 1,
      explanation: "<code>_Static_assert(sizeof(int) == 4, \"int must be 4 bytes\")</code> 在<strong>编译期</strong>求值，若条件为假则产生编译错误并输出指定消息。适合验证类型大小、结构体布局等编译期假设。"
    },
    {
      type: "output",
      question: "以下C11代码的输出是什么？<pre class=\"language-c\">#define type_name(x) _Generic((x), \\\n    int: \"int\", \\\n    double: \"double\", \\\n    default: \"other\")\n\nint main(void) {\n    double d = 3.14;\n    printf(\"%s\\n\", type_name(d));\n    return 0;\n}</pre>",
      options: [
        "int",
        "double",
        "other",
      ...
      answer: 1,
      explanation: "<code>_Generic</code> 根据表达式的<strong>类型</strong>在编译时选择对应的值。变量 <code>d</code> 的类型是 <code>double</code>，所以匹配 <code>double: \"double\"</code> 分支，输出 <code>double</code>。"
    },
    {
      type: "choice",
      question: "C11 中 <code>_Alignof(type)</code> 的作用是：",
      options: [
        "返回类型的大小（字节数）",
        "返回类型的对齐要求（字节数）",
        "设置变量的对齐方式",
      ...
      answer: 1,
      explanation: "<code>_Alignof</code>（或 <code>alignof</code>）返回类型的<strong>对齐要求</strong>，即该类型变量的地址应是多少字节的倍数。例如 <code>_Alignof(double)</code> 通常为 8。<code>_Alignas</code> 则用于<strong>设置</strong>对齐方式。"
    },
    {
      type: "truefalse",
      question: "C11 允许在结构体中使用匿名结构体或匿名联合体，直接通过外层结构体访问其成员。",
      options: ["正确", "错误"],
      answer: 0,
      explanation: "C11 正式支持匿名结构体/联合体。例如 <code>struct S { union { int i; float f; }; };</code>，可以直接用 <code>s.i</code> 和 <code>s.f</code> 访问，无需经过中间名称，简化了嵌套结构体的使用。"
    },
    {
      type: "truefalse",
      question: "用 <code>_Noreturn</code> 修饰的函数必须不返回到调用者（如内部调用 <code>exit()</code> 或无限循环）。",
      options: ["正确", "错误"],
      answer: 0,
      explanation: "<code>_Noreturn</code>（或 <code>noreturn</code>）告诉编译器该函数永远不会返回。典型用途是 <code>exit()</code>、<code>abort()</code> 的包装函数或无限循环函数。如果标记了 <code>_Noreturn</code> 却实际返回了，则是未定义行为。"
    },
    {
      type: "output",
      question: "以下C11代码的输出是什么？<pre class=\"language-c\">#include &lt;stdatomic.h&gt;\n#include &lt;stdio.h&gt;\n\nint main(void) {\n    _Atomic int counter = 0;\n    atomic_fetch_add(&amp;counter, 5);\n    atomic_fetch_sub(&amp;counter, 2);\n    printf(\"%d\\n\", counter);\n    return 0;\n}</pre>",
      options: [
        "0",
        "5",
        "3",
      ...
      answer: 2,
      explanation: "<code>_Atomic int</code> 声明原子整型变量。<code>atomic_fetch_add</code> 原子地加 5（0→5），<code>atomic_fetch_sub</code> 原子地减 2（5→3），最终输出 <strong>3</strong>。原子操作保证多线程下的数据安全。"
    }
  ]
}
];
