...
<p><code>auto</code>让编译器推导类型，减少冗长声明。范围for循环简化容器遍历。<code>nullptr</code>是类型安全的空指针。<strong>移动语义</strong>通过"窃取"资源而非复制来提升性能，使用<code>&amp;&amp;</code>定义移动构造和移动赋值。Lambda表达式<code>[捕获](参数){函数体}</code>可作为简洁的内联函数对象使用。</p>
  ...
  questions: [
    ...
    },
    {
      type: "choice",
      difficulty: "intermediate",
      question: "C++17中<code>std::optional</code>最适合替代以下哪种模式？",
      options: ["使用异常处理错误", "使用特殊返回值（如-1或nullptr）表示\"无值\"", "使用全局变量存储状态", "使用模板特化"],
      answer: 1,
      explanation: "<code>std::optional&lt;T&gt;</code>用于表示一个值可能存在也可能不存在的情况，替代了过去用特殊值（如<code>-1</code>、<code>nullptr</code>、<code>\"\"</code>）表示\"无值\"的做法。可通过<code>has_value()</code>或隐式bool转换检查是否有值。"
    },
    {
      type: "choice",
      difficulty: "intermediate",
      question: "关于C++17的<code>std::variant</code>，以下哪项描述是正确的？",
      options: ["它与C语言的union完全相同", "它是类型安全的，可以存储预定义类型列表中的任意一种类型的值", "它可以同时存储多个不同类型的值", "它只能存储基本数据类型"],
      answer: 1,
      explanation: "<code>std::variant&lt;int, double, std::string&gt;</code>在任意时刻只存储一个值，但该值的类型可以是模板参数列表中的任意一种。与C的<code>union</code>不同，<code>variant</code>是<strong>类型安全</strong>的——它记录当前存储的是哪种类型，访问错误类型会抛出<code>std::bad_variant_access</code>异常。"
    },
    {
      type: "output",
      difficulty: "intermediate",
      question: "以下C++17代码的输出是什么？<pre class=\"language-cpp\"><code>std::pair&lt;int, double&gt; p{42, 3.14};\nauto [x, y] = p;\nstd::cout &lt;&lt; x &lt;&lt; \" \" &lt;&lt; y;\n</code></pre>",
      options: ["42 3.14", "编译错误", "0 0", "42 3"],
      answer: 0,
      explanation: "C++17的<strong>结构化绑定</strong>（structured bindings）允许用<code>auto [x, y] = p;</code>将<code>pair</code>的两个成员分别绑定到<code>x</code>和<code>y</code>。因此输出<strong>42 3.14</strong>。这也适用于<code>tuple</code>、数组和具有公有成员的结构体。"
    },
    {
      type: "choice",
      difficulty: "advanced",
      question: "C++17中<code>if constexpr</code>与普通<code>if</code>语句的关键区别是什么？",
      options: ["if constexpr只能在类内使用", "if constexpr在编译时求值，不满足条件的分支不会被编译", "if constexpr的条件只能是bool字面值", "if constexpr运行速度更快但功能与if相同"],
      answer: 1,
      explanation: "<code>if constexpr</code>在<strong>编译时</strong>求值条件表达式。不满足条件的分支会被丢弃（不参与编译），这在模板元编程中非常有用，可以避免对不适用类型实例化无效代码。"
    },
    {
      type: "truefalse",
      difficulty: "basic",
      question: "C++17的<code>std::filesystem</code>库需要依赖第三方库（如Boost.Filesystem）才能使用。",
      options: ["正确", "错误"],
      answer: 1,
      explanation: "<code>std::filesystem</code>是C++17标准库的一部分，包含在<code>&lt;filesystem&gt;</code>头文件中，<strong>不需要</strong>第三方库。它提供了路径操作、文件状态查询、目录遍历、文件复制/删除等功能。其API设计确实参考了Boost.Filesystem，但现在已是标准的一部分。"
    },
    {
      type: "choice",
      difficulty: "advanced",
      question: "使用<code>std::string_view</code>而非<code>const std::string&amp;</code>作为只读参数的主要优势是什么？",
      options: ["string_view可以修改原字符串", "string_view可以避免从C字符串字面值创建临时std::string对象", "string_view支持正则表达式", "string_view自动管理内存"],
      answer: 1,
      explanation: "<code>std::string_view</code>是一个<strong>非拥有的</strong>字符串引用（指针+长度），不分配内存。当函数参数为<code>const std::string&amp;</code>时，传入C字符串字面值<code>\"hello\"</code>会隐式构造临时<code>std::string</code>对象；而<code>string_view</code>可以直接引用该字面值，实现<strong>零拷贝</strong>。"
    },
    {
      type: "output",
      difficulty: "advanced",
      question: "以下C++17代码中<code>p</code>的推导类型是什么？<pre class=\"language-cpp\"><code>std::pair p{1, 2.0};</code></pre>",
      options: ["编译错误，pair必须显式指定模板参数", "std::pair&lt;int, double&gt;", "std::pair&lt;int, int&gt;", "std::pair&lt;double, double&gt;"],
      answer: 1,
      explanation: "C++17引入了<strong>类模板参数推导</strong>（CTAD），编译器可以从构造函数的实参推导模板参数。<code>std::pair p{1, 2.0};</code>中<code>1</code>是<code>int</code>，<code>2.0</code>是<code>double</code>，因此推导为<code>std::pair&lt;int, double&gt;</code>。"
    },
    {
      type: "choice",
      difficulty: "intermediate",
      question: "C++20中Concepts的主要作用是什么？",
      options: ["替代所有模板", "为模板参数添加编译时约束，提供更清晰的错误信息", "实现运行时多态", "自动生成文档"],
      answer: 1,
      explanation: "<strong>Concepts</strong>允许为模板参数指定<strong>编译时约束</strong>（如要求类型可排序、可哈希等）。当约束不满足时，编译器给出清晰的错误信息，而非传统模板的冗长错误。语法如<code>template&lt;std::integral T&gt;</code>或<code>requires</code>子句。"
    },
    {
      type: "choice",
      difficulty: "advanced",
      question: "以下C++20 Ranges代码中，<code>|</code>（管道符）的作用是什么？<pre class=\"language-cpp\"><code>auto result = nums | std::views::filter(is_even)\n                  | std::views::transform(square);</code></pre>",
      options: ["按位或运算", "将多个视图适配器组合成管道链", "逻辑或运算", "字符串拼接"],
      answer: 1,
      explanation: "C++20 Ranges库中<code>|</code>被重载为<strong>管道运算符</strong>，用于将多个视图适配器串联起来形成处理管道。数据从左到右流过每个适配器：先过滤偶数，再对结果进行平方变换。视图是<strong>惰性求值</strong>的，不会立即计算。"
    },
    {
      type: "truefalse",
      difficulty: "intermediate",
      question: "C++20的协程（coroutines）使用操作系统线程来实现并发。",
      options: ["正确", "错误"],
      answer: 1,
      explanation: "C++20协程是<strong>无栈协程</strong>（stackless coroutines），它们通过编译器将函数转换为状态机来实现挂起/恢复，<strong>不依赖操作系统线程</strong>。协程在被挂起时保存状态到堆上，恢复时从该状态继续执行。关键字包括<code>co_await</code>、<code>co_yield</code>和<code>co_return</code>。"
    },
    {
      type: "output",
      difficulty: "intermediate",
      question: "以下C++20代码的输出是什么？<pre class=\"language-cpp\"><code>std::string s = std::format(\"{} + {} = {}\", 1, 2, 1+2);\nstd::cout &lt;&lt; s;\n</code></pre>",
      options: ["1 + 2 = 3", "{} + {} = {}", "1 + 2 = 12", "编译错误"],
      answer: 0,
      explanation: "C++20的<code>std::format</code>提供<strong>类型安全</strong>的格式化，类似Python的<code>str.format()</code>。<code>{}</code>是占位符，依次替换为对应参数。<code>1+2</code>在传参时已计算为<code>3</code>，所以输出<strong>1 + 2 = 3</strong>。"
    },
    {
      type: "choice",
      difficulty: "advanced",
      question: "C++20中三路比较运算符<code>&lt;=&gt;</code>（太空船运算符）的返回类型可能是以下哪种？",
      options: ["bool", "int", "std::strong_ordering等比较类别类型", "std::variant"],
      answer: 2,
      explanation: "<code>&lt;=&gt;</code>返回的是<strong>比较类别类型</strong>，如<code>std::strong_ordering</code>、<code>std::weak_ordering</code>或<code>std::partial_ordering</code>。这些类型可以与<code>0</code>比较来判断小于/等于/大于关系。编译器还可以自动从<code>&lt;=&gt;</code>生成<code>&lt;</code>、<code>&gt;</code>、<code>&lt;=</code>、<code>&gt;=</code>等运算符。"
    },
    {
      type: "choice",
      difficulty: "intermediate",
      question: "C++20模块（modules）相比传统的<code>#include</code>有什么优势？",
      options: ["模块只能包含函数声明", "模块避免了头文件的重复解析，加快编译速度并消除宏泄漏", "模块让代码在运行时动态加载", "模块替代了命名空间"],
      answer: 1,
      explanation: "C++20<strong>模块</strong>用<code>import</code>代替<code>#include</code>。模块只需编译一次，避免了头文件被每个翻译单元重复解析的开销；模块内部的宏不会泄漏到导入方，解决了头文件中宏污染的问题。使用<code>export module</code>声明模块，<code>import</code>导入模块。"
    },
    {
      type: "truefalse",
      difficulty: "intermediate",
      question: "C++20中<code>consteval</code>函数可以在运行时被调用。",
      options: ["正确", "错误"],
      answer: 1,
      explanation: "<code>consteval</code>（即时函数）<strong>必须</strong>在编译时求值，不能在运行时调用——如果无法在编译时求值则编译失败。这与<code>constexpr</code>不同，<code>constexpr</code>函数<em>可以</em>在编译时或运行时调用。<code>constinit</code>则确保变量在编译时初始化，但之后可在运行时修改。"
    },
    {
      type: "choice",
      difficulty: "advanced",
      question: "C++23中<code>std::expected&lt;T, E&gt;</code>的设计目的是什么？",
      options: ["替代std::optional，支持多个值", "提供一种不使用异常的错误处理方式，同时携带错误信息", "实现异步编程", "替代std::variant"],
      answer: 1,
      explanation: "<code>std::expected&lt;T, E&gt;</code>存储一个期望值<code>T</code>或一个错误值<code>E</code>。相比<code>std::optional</code>只能表示\"有值/无值\"，<code>expected</code>在无值时还能携带<strong>错误信息</strong>；相比异常，它使用返回值传递错误，开销更可预测且控制流更显式。"
    },
    {
      type: "choice",
      difficulty: "intermediate",
      question: "C++23的<code>std::print</code>相比<code>std::cout &lt;&lt;</code>的主要改进是什么？",
      options: ["运行速度慢但更安全", "结合了std::format的格式化语法，支持类型安全的直接输出", "只能输出字符串类型", "需要手动刷新缓冲区"],
      answer: 1,
      explanation: "<code>std::print(\"Hello, {}!\\n\", name)</code>结合了<code>std::format</code>的格式化能力和直接输出功能，语法简洁，<strong>类型安全</strong>，并正确处理Unicode。它解决了<code>cout</code>链式<code>&lt;&lt;</code>操作冗长且格式控制不便的问题。"
    },
    {
      type: "choice",
      difficulty: "advanced",
      question: "C++23的\"Deducing this\"特性允许什么？",
      options: ["自动推导类的构造函数参数", "在成员函数中显式声明this参数，从而推导对象的值类别", "自动生成this指针", "替代虚函数实现多态"],
      answer: 1,
      explanation: "\"Deducing this\"允许将<code>this</code>作为显式的函数参数，如<code>void foo(this auto&amp;&amp; self)</code>。编译器会根据调用对象的值类别（左值/右值、const/非const）推导<code>self</code>的类型，从而<strong>消除</strong>为不同const/引用限定符重复编写成员函数重载的需求。"
    },
    {
      type: "truefalse",
      difficulty: "intermediate",
      question: "RAII（资源获取即初始化）模式要求在构造函数中获取资源，在析构函数中释放资源。",
      options: ["正确", "错误"],
      answer: 0,
      explanation: "<strong>RAII</strong>的核心思想确实是：在对象<strong>构造</strong>时获取资源（如打开文件、分配内存、获取锁），在<strong>析构</strong>时释放资源。由于C++保证局部对象离开作用域时析构函数一定会被调用（即使发生异常），RAII确保了资源不会泄漏。智能指针、<code>std::lock_guard</code>等都是RAII的经典应用。"
    },
    {
      type: "choice",
      difficulty: "intermediate",
      question: "以下关于智能指针的说法，哪个是正确的？",
      options: ["std::unique_ptr允许多个指针共享同一对象的所有权", "std::shared_ptr使用引用计数，当最后一个shared_ptr销毁时释放对象", "std::weak_ptr会阻止对象被释放", "std::shared_ptr不能用于多态（基类指针指向派生类）"],
      answer: 1,
      explanation: "<code>std::shared_ptr</code>通过<strong>引用计数</strong>实现共享所有权，每复制一次计数加1，每销毁一个计数减1，减到0时释放对象。<code>unique_ptr</code>是独占所有权，不能复制。<code>weak_ptr</code>是弱引用，<strong>不</strong>增加引用计数，不阻止对象释放，用于打破循环引用。"
    },
    {
      type: "output",
      difficulty: "advanced",
      question: "以下代码存在什么问题？<pre class=\"language-cpp\"><code>std::string s = \"hello\";\nstd::string s2 = std::move(s);\nstd::cout &lt;&lt; s.size();\n</code></pre>",
      options: ["程序一定崩溃", "输出5", "s处于有效但未指定状态，s.size()的结果不确定（可能为0）", "编译错误"],
      answer: 2,
      explanation: "调用<code>std::move(s)</code>后，<code>s</code>的资源被移动到<code>s2</code>中。根据标准，<code>s</code>处于<strong>有效但未指定</strong>（valid but unspecified）的状态。对它调用<code>size()</code>不会崩溃（仍然有效），但返回值不确定——大多数实现中为<code>0</code>，但标准不保证。这就是<strong>use-after-move</strong>陷阱。"
    }
  ]
}
];
